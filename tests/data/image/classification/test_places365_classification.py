...
from gate.data.image.classification.places365 import (
    build_places365_dataset,
)


def test_build_places365_dataset():
    # Test if the function returns the correct dataset split

    train_set = build_places365_dataset(
        "train", data_dir=os.environ.get("PYTEST_DIR")
    )
    assert train_set is not None, "Train set should not be None"

    val_set = build_places365_dataset(
        "val", data_dir=os.environ.get("PYTEST_DIR")
    )
    assert val_set is not None, "Validation set should not be None"

    test_set = build_places365_dataset(

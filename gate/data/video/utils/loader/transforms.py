...
class GroupOneOfFiveCrops:
    ...
    def __call__(self, img_group):
        ...
        ret = []
        for img in img_group:
            offset_w, offset_h = GroupOneOfFiveCrops.crop_offset(
...
class GroupScale:
    ...
    def __init__(self, size, interpolation=Image.BILINEAR):
        ...
        self.worker = torchvision.transforms.Resize(
            size, _interpolation_modes_from_int(interpolation)
        )
...
class GroupSqueezeScale:
    """Rescales the input PIL.Image to the given 'size x size'.
    size: number of pixels of one edge
    interpolation: Default: PIL.Image.BILINEAR
    """
    ...
    def __init__(
        ...
        if isinstance(size, int):
            ...
            raise TypeError("size must be int or tuple[int, int]")
...
class GroupMultiply:
    def __init__(self, factor: float | list[float]):
        ...
        # convert to 12-element tuple
        self.matrix = tuple(self.matrix.flatten().tolist())

    def __call__(self, img_group):
        return [img.convert("RGB", self.matrix) for img in img_group]


class GroupRandomMultiply:
    """Randomly multiply the image"""

    def __init__(self, factor_min: float, factor_max: float):
        self.factor_min = factor_min
        self.factor_max = factor_max

    def __call__(self, img_group):
        v = random.random()
        factor = v * (self.factor_max - self.factor_min) + self.factor_min
        return GroupMultiply(factor)(img_group)


class GroupRandomScale:
    ...
    def __call__(self, img_group):
        ...
        return GroupScale(size)(img_group)

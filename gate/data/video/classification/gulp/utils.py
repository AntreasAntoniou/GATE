...
def spatial_sampling_5(
    ...
    if spatial_idx == -1:
        ...
        frames, _, x_offset, y_offset = transform.uniform_crop_5(
            frames, crop_size, spatial_idx % 5
        )
        if spatial_idx >= 5:
...
def strided_frame_indices(
    ...
    if num_video_frames < sample_span:
        ...
        video_frame_indices = list(
            itertools.chain.from_iterable(
                itertools.repeat(x, num_repeats) for x in video_frame_indices
            )
        )
    ...
    sampled_frame_indices = [
        ...
        for idx in range(
            start_idx, start_idx + sampling_rate * num_sample_frames, sampling_rate
        )
    ]
...
def sparse_frame_indices(
    num_input_frames, num_output_frames, uniform=True, num_neighbours=1
):
    """
    ...
    if num_input_frames_wo_neighbour_length < num_output_frames:
        num_repeats = (
            num_output_frames // num_input_frames_wo_neighbour_length
        ...
        num_input_frames_wo_neighbour_length *= (
            num_repeats  # equal to: len(video_frame_indices)
        )

    segment_start_indices = (
        []
    )  # [segment_start_indices[i], segment_start_indices[i+1]) is the range to sample 1 frame (snippet) from the i-th segment.
    for output_frame_id in range(num_output_frames):
        segment_start_indices.append(
            int(
                num_input_frames_wo_neighbour_length
                * output_frame_id
                / num_output_frames
    ...
    if uniform:
        sampled_frame_indices = [
            video_frame_indices[idx] for idx in segment_start_indices
        ]
    else:
        # Random sampling from each segment
        segment_start_indices.append(
            num_input_frames_wo_neighbour_length
        )  # since last frame is num_input_frames_wo_neighbour_length - 1
        frame_indices = []
        for output_frame_id in range(num_output_frames):
...
def TSN_sample_indices(num_input_frames, num_output_frames, mode="train", new_length=1):
    ...
    if mode == "train":
        ...
        if average_duration > 0:
            ...
            ) + np.random.randint(average_duration, size=num_output_frames)
        elif num_input_frames > num_output_frames:
            offsets = np.sort(
                np.random.randint(
                    num_input_frames - new_length + 1, size=num_output_frames
                )
            )
        else:
            logger.warning(
                "Cannot sample %d frames from %d. Returning all first frames.",
                num_output_frames,
                num_input_frames,
            )
...
def TDN_sample_indices(num_input_frames, num_output_frames, mode="train", new_length=5):
    if mode == "train":  # TSN uniformly sampling for TDN
        ...
        if average_duration > 0:
            offsets += list(
                np.multiply(list(range(num_output_frames)), average_duration)
                + randint(average_duration, size=num_output_frames)
            )
        elif num_input_frames > num_output_frames:
            if (num_input_frames - new_length + 1) >= num_output_frames:
                offsets += list(
        ...
        offsets = (
            ...
            + [
                (idx * t_stride + start_idx2) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx3) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx4) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx5) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx6) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx7) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx8) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx9) % num_input_frames
                for idx in range(num_output_frames)
            ]
            + [
                (idx * t_stride + start_idx10) % num_input_frames
                for idx in range(num_output_frames)
            ]
        )
    else:
    ...
    for seg_ind in offsets:
        ...
        for i in range(0, new_length, 1):
            frames_idx.append(p)
            if (
                p < num_input_frames - 1
            ):  # if p is the last frame already, do not increase
                p += 1
